...
describe('harcon', function () {
	...
	let realDivision = harconName + '.sub'

	describe('Test Harcon system calls', function () {
		it('Retrieve divisions...', async function () {
			let divisions = await inflicter.divisions()
			expect( divisions.sort() ).to.eql( [ harconName, harconName + '.sub', harconName + '.sub.click', harconName + '.sub.maison.cache' ] )
		})
	...
	describe('simple messages', function () {
		it('Alize dormir', async function () {
			let res = await inflicter.ignite( clerobee.generate(), null, realDivision, 'Alizee.dormir' )
			expect(res).to.eql( 'Non, non, non!' )
		})
		it('Alize flegme', async function () {
			this.timeout(5000)
			let res = await inflicter.ignite( clerobee.generate(), null, realDivision, 'Alizee.flegme' )
			expect(res).to.eql( 'Quoi?' )
		})
		it('Alize superFlegme', async function () {
			this.timeout(5000)
			try {
				await inflicter.ignite( clerobee.generate(), null, realDivision, 'Alizee.superFlegme' )
				assert.fail( 'Should not be here...' )
			} catch (err) { expect(err).to.be.an.instanceof( Error ) }
		})
		it('Boss shrinking', async function () {
			this.timeout(5000)
			let res = await inflicter.ignite( clerobee.generate(), null, realDivision, 'Boss.shrink', 'hello?' )
	...
	describe('Depth handling', function () {
		it('multilevel divisions', async function () {
			let res = await inflicter.ignite( clerobee.generate(), null, 'HarconSys.sub.maison.cache', 'Margot.alors' )
			expect(res).to.eql( 'Oui?' )
		})
		it('multilevel contextes', async function () {
			let res = await inflicter.ignite( clerobee.generate(), null, 'HarconSys.sub.maison.cache', 'paresseux.fille.alors' )
			expect(res).to.eql( 'Oui?' )
		})
	})
	...
	describe('Error handling', function () {
		it('Throw error', async function () {
			try {
				await inflicter.ignite( clerobee.generate(), null, realDivision, 'Bandit.delay' )
				assert.fail( 'Should not be here...' )
			} catch (err) { expect(err).to.be.an.instanceof( Error ) }
		})
	})
	...
	describe('State shifting', function () {
		it('Simple case', async function () {
			...
			await inflicter.ignite( clerobee.generate(), null, realDivision, 'Marie.notify', 'data', realDivision, 'Lina.marieChanged')

			await Proback.timeout( 250 )
			await inflicter.ignite( clerobee.generate(), null, realDivision, 'Marie.simple', 'Bonjour', 'Salut' )
	...
	describe('Harcon distinguish', function () {
		it('Access distinguished entity', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'Charlotte.access')
				should.exist(res)
		...
		it('Access distinguished entity', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'Charlotte-Unique.access')
				should.exist(res)
				expect( res ).to.include( 'D\'accord?' )
			} catch ( err ) { console.error(err) }
		})
	...
	describe('Erupt flow', function () {
		it('Simple greetings by name is', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'Marie.simple', 'whatsup?', 'how do you do?')
				let res2 = await inflicter.ignite( '0', null, realDivision, 'greet.simple', 'whatsup?', 'how do you do?')
		...
		it('Marion', async function () {
			...
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'Marion.force' )
				should.exist(res)
	...
	describe('Harcon workflow', function () {
		...
		it('Simple greetings is', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'greet.simple', 'whatsup?', 'how do you do?')
				should.exist(res)
		...
		it('Morning greetings is', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'morning.wakeup')
				expect(res).to.eql( [ 'Hi there!', 'My pleasure!' ] )
			} catch (err) { console.error(err) }
		})
		it('General dormir', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'morning.dormir')
				expect(res).to.eql( [ 'Non, non, non!', 'Non, Mais non!' ] )
			} catch (err) { console.error(err) }
		})
		it('Specific dormir', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'morning.girls.dormir')
				expect(res).to.eql( [ 'Non, non, non!', 'Non, Mais non!' ] )
			} catch (err) { console.error(err) }
		})
		it('No answer', async function () {
			try {
				await inflicter.ignite( '0', null, realDivision, 'cave.echo')
			} catch (err) {
		...
		it('Division Promise test', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision + '.click', 'greet.simple', 'Hi', 'Ca vas?' )
				should.exist(res)
		...
		it('Domina', async function () {
			try {
				let res = await inflicter.ignite( '0', null, realDivision, 'Domina.force')
				should.exist(res)
	...
	describe('Live reload test', function () {
		it('Changing Alizee', async function () {
			...
			try {
				...
				let res = await inflicter.ignite( '0', null, realDivision, 'Lina.flying')
