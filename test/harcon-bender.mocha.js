...
describe('HarconBend', function () {
	...
	describe('Test completeness', function () {
		it('Firebender', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.completeness' )
	...
	describe('Bending', function () {
		it('Spread', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.rever', [ 'bonne nuite' ] )
		...
		it('Series', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.repose', [ 'bonne soirree' ])
				expect( res ).to.eql( [ 'Merci', 'Enchentée.' ] )
			} catch (err) { console.error( err ) }
		})
		...
		it('Series with validation failuer', async function () {
			try {
				await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.repose', [ 'bonne nuite' ] )
		...
		it('Waterfall', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.chouchou', [ 'bonne nuite' ] )
		...
		it('Series wizh skipif', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.choisi', [ 'bonne nuite' ] )
		...
		it('Forech Series', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.distribute', [ ])
		...
		it('Foreach Waterfall', async function () {
			try {
				let res = await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Julie.waterfall', [ ] )
	...
	describe('Bending errors', function () {
		it('Timeout', async function () {
			this.timeout(3500)
			try {
				await inflicter.ignite( clerobee.generate(), null, '', 'FireBender.exec', '', 'Alizee.superFlegme', [ ] )
				assert.fail( 'Should not be here...' )
			} catch (err) { expect(err).to.be.an.instanceof( Error ) }
		})
	})
