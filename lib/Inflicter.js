...
function Inflicter ( options ) {
	...
	self.purifyConfig = { arrayMaxSize: self.options.arrayMaxSize || 100, maxLevel: self.options.maxLevel || 2 }
...
inflicter.sendNotification = async function ( payload, events ) {
	...
	for (let event of events) {
		await self.require( null, null, event.division || self.division, event.event, payload )
	}
	return OK
}
...
*/
inflicter.require = async function ( ...parameters ) {
	return this.systemFirestarter.ignite( Communication.MODE_REQUIRE, ...parameters )
}
