...
var VERSION = exports.VERSION = '3.9.0'
...
function Inflicter ( options, callback ) {
	...
	self.logger.harconlog = function ( err, message, obj, level ) {
		this.log( err ? 'error' : (level || 'debug'), err ? err.message : message, assigner.assign( purifyObj(obj, self.purifyConfig, 0, []), { 'harcon': VERSION } ) )
	}.bind( self.logger )
	...
	var barrelConfig = assigner.assign(
		this.options.barrel || {},
		{ name: this.name, division: this.division, logger: this.logger, connectedDivisions: this.connectedDivisions }
	)
	...
	return new Promise( function (resolve, reject) {
		...
		.then( function () {
			...
			if ( object.init && _.isFunction( object.init ) ) {
				fns.push( function (cb) {
					var componentConfig = assigner.assign( {}, self.options.millieu, options || self.options[fss.name] || self.options.fireContext || {} )
