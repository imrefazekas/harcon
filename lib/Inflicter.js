...
var VERSION = exports.VERSION = '4.6.6'
	...
	if ( _.isObject(obj) ) {
		...
		for (var key in obj)
			if ( key && obj[key] ) {
				...
				path.push( obj[key] )
				var resObj = level > config.maxLevel ? '...' : purify( obj[key], config, level + 1, path )
				if (resObj)
					res[key] = resObj
			}
		return res
...
function Inflicter ( options, callback ) {
	...
	var barrelConfig = assigner.assign(
		this.options.barrel || {},
		{ name: this.name, division: this.division, logger: this.logger, connectedDivisions: this.connectedDivisions, igniteLevel: this.options.igniteLevel, seals: this.options.seals }
	)
...
inflicter.entities = function ( division, callback ) {
	...
	return new Promise( function (resolve, reject) {
		var entities = self.barrel ? self.barrel.entities( division ) : []
