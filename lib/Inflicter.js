...
inflicter.init = async function () {
	...
	let barrelConfig = assigner.assign(
		self.options.barrel || {},
		{ name: self.name, division: self.division, logger: self.logger, connectedDivisions: self.connectedDivisions, igniteLevel: self.options.igniteLevel, seals: self.options.seals, unfoldAnswer: self.options.unfoldAnswer, _systemoptions: self.options }
	)
	...
	if (self.options.mortar.enabled) {
		try {
			...
			await self.addicts( Mortar, self.options.mortar )
