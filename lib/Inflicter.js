...
inflicter.init = async function () {
	...
	let barrelConfig = assigner.assign(
		self.options.barrel || {},
		{ name: self.name, division: self.division, subDivision: self.subDivision, logger: self.logger, connectedDivisions: self.connectedDivisions, igniteLevel: self.options.igniteLevel, seals: self.options.seals, unfoldAnswer: self.options.unfoldAnswer, idLength: self.options.idLength, _systemoptions: self.options }
	)
...
inflicter.sendNotification = async function ( payload, events ) {
	...
	for (let event of events) {
		await self.ignite( null, null, event.division || self.division, event.event, payload )
	}
	return OK
}
...
* @param {Object} object Object
*/
inflicter.addicts = async function ( object, options ) {
	let self = this

	await self.detracts( object )

	let sub = self.options.subDivision ? '.' + self.options.subDivision : ''

	if (!self.options.suppressing && object.division ) {
		if ( object.division !== self.division && !object.division.startsWith( self.division + '.' ) )
			object.division = self.division + sub + '.' + object.division
	}
...
*/
inflicter.simpleIgnite = async function ( ) {
...
* @param {String} params A vararg element possessing the objects to be sent with the message. Can be empty
*/
inflicter.ignite = async function ( ) {
