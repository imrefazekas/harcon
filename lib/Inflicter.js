...
var VERSION = exports.VERSION = '2.10.0';
...
function Inflicter( options, callback ){
	...
	this.barrel.init( extend( this.options.barrel || {}, { name: this.name, division: this.division, logger: this.logger } ), function(err){
		...
		self.systemFirestarter = self.barrel.systemFirestarter = self.addicts( {
			...
			setTimeout( function(){ callback(err, res); }, 1 );
		} );
...
inflicter.divisions = function( callback ){
	var divisions = this.barrel.divisions( );
	if( callback )
...
inflicter.detracts = function( object, callback ){
	return this.barrel.castOf( object.name, callback );
};
