...
var VERSION = exports.VERSION = '2.3.0';
...
inflicter.addicts = function( object, options, callback ){
	...
	return this.barrel.affiliate( fss, function(err){
		if( callback )
			return callback( err, {
				name: fss.name, context: fss.context, division: fss.division, services: fss.services(), rest: !!object.rest, websocket: !!object.websocket
			} );
		if( err )
...
inflicter.addict = function( division, name, eventName, fn ){
	var flamestarter = new this.Flamestarter( this.options, this.barrel, division || this.division, name, eventName, fn, this.logger );
