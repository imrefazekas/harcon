...
commPrototype.burst = function ( externalID, flowID, sourceDivision, source, division, event, params, callback ) {
	var comm = new Communication( null, flowID || this.flowId, externalID || this.externalId, sourceDivision, source, division || this.division, event, params, callback )
...
exports.newCommunication = function ( originalId, flowId, externalId, sourceDivision, source, division, event, params, callback, responder, error, response ) {
	var valveIndex = event.indexOf( VALVE_SEPARATOR )
	var valve = VALVE_DEFAULT
	if ( valveIndex > -1 ) {
		valve = event.substring( valveIndex + 1 )
		event = event.substring( 0, valveIndex )
	}

	var c = new Communication( originalId, flowId, externalId, sourceDivision, source, division, event, params, callback, responder, error, response )
	c.valve = valve
	return c
}
