...
commPrototype.spread = function ( ) {
	var comm = new Communication( this.originalId, this.flowId, this.externalId, this.sourceDivision, this.source, this.division, this.event, this.params )
	comm.id = this.id
	comm.date = this.date
...
commPrototype.twist = function ( name, error, response ) {
	var comm = new Communication( this.id, this.flowId, this.externalId, this.sourceDivision, this.source, this.division, this.event, this.params, this.callback, name, error, response )
	return comm
}
...
*/
commPrototype.burst = function ( externalID, sourceDivision, source, division, event, params, callback ) {
	var comm = new Communication( null, this.flowId, externalID || this.externalId, sourceDivision, source, division || this.division, event, params, callback )
...
exports.newCommunication = function ( originalId, flowId, externalId, sourceDivision, source, division, event, params, callback, responder, error, response ) {
	return new Communication( originalId, flowId, externalId, sourceDivision, source, division, event, params, callback, responder, error, response )
}

exports.importCommunication = function ( obj ) {
	var c = new Communication( obj.originalId, obj.flowId, obj.externalId, obj.sourceDivision, obj.source, obj.division, obj.event, obj.params, obj.callback, obj.responder, obj.error, obj.response )
	c.id = obj.id
	c.terms = obj.terms
	if ( obj.expose )
		c.expose = true
