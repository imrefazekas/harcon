...
module.exports = {
	...
	validateFlow: async function (primer, flowId, err, res) {
		...
		if ( !self.defs[primer] ) {
			let entities = await self.require( '', flowId, self.division, 'Inflicter.entities', '' )
			for ( let entity of entities ) {
				if (err)
					await self.require( '', flowId, entity.division, entity.name + '.flowFailed', flowId, errMessage )
				else
					await self.require( '', flowId, entity.division, entity.name + '.flowSucceeded', flowId, res )
			}
		}
		return OK
	},
	...
	performStep: async function ( primer, res, terms, ignite ) {
		...
		if ( primer.fragmented ) {
			await self.require( terms.sourceComm.externalId, terms.sourceComm.flowId, primer.division || self.division, primer.event || primer, res )
	...
	simpleFire: async function ( primer, res, terms, ignite ) {
		...
		return await self[ primer.foreach ? 'performEachStep' : 'performStep']( primer, res, terms, ignite )
	},
	seriesFire: async function ( primers, data, terms, ignite ) {
		let self = this
	...
	waterfallFire: async function ( primers, data, terms, ignite ) {
		let self = this
