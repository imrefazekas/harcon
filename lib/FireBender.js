...
module.exports = {
	...
	validateFlow: function (primer, flowId, err, res) {
		let self = this
		self.harconlog( null, 'Flow terminated', { flowId: flowId }, 'silly' )
		if ( !self.defs[primer] ) {
			self.firestarter.barrel.systemFirestarter
			self.ignite( '', flowId, self.division, 'Inflicter.entities', '', function (err, entities) {
				if (err) return self.harconlog(err)
				entities.forEach( function (entity) {
					self.ignite( '', flowId, entity.division, entity.name + '.flowTerminated', flowId, err, res )
				} )
			} )
		}
	},
