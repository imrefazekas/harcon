...
module.exports = {
		...
		primers.forEach( function (primer) {
			if (fns.length === 0) {
				...
				fns.push( function (res, cb) {
					ignite( terms.sourceComm.externalId, terms.sourceComm.flowId, primer.division || self.division, primer.event || primer, self.options.unfoldAnswer ? res : res[0], cb )
				} )
			}
		} )
	...
	exec: function ( division, event, parameters, terms, ignite, callback ) {
		...
		as.push( function (err, res) {
			...
			self[ action.type + 'Fire' ]( action.primers || [], self.options.unfoldAnswer ? res : res[0], terms, ignite, callback )
		} )
