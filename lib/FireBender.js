...
module.exports = {
	...
	performStep: async function ( primer, res, terms ) {
		...
		return await terms.request( terms.sourceComm.externalId, terms.sourceComm.flowId, primer.division || self.division, primer.event || primer, res )
	},
	...
	simpleFire: async function ( primer, res, terms ) {
		...
		return await self[ primer.foreach ? 'performEachStep' : 'performStep']( primer, res, terms )
	},
	...
	exec: async function ( division, event, parameters, terms ) {
		...
		return await self[ (action.type || 'series') + 'Fire' ]( action.primers || [], self.options.unfoldAnswer ? res : res[0], terms )
	}
}
