...
barrel.affiliate = function ( firestarter, callback ) {
	...
	self.igniteSystemEvent( 'affiliate', firestarter )

	self.firestarters.push( firestarter )

	return new Promise( (resolve, reject) => {
		self.setWarper( firestarter.division, liner)
		.then( () => { return self.newEntity( firestarter.division || self.systemFirestarter.division, firestarter.context, firestarter.name) } )
		.then( () => { return firestarter.distinguishedName ? self.newEntity( firestarter.division || self.systemFirestarter.division, firestarter.context, firestarter.distinguishedName) : 'ok' } )
		.then( () => {
			self.igniteSystemEvent( 'affiliated', firestarter.division, firestarter.context, firestarter.name )

			if (callback) callback(null, firestarter)
			resolve(firestarter)
		} )
		.catch( (reason) => {
			if (callback) callback(reason)
...
barrel.matching = function (comm) {
	...
	this.logger.harconlog( null, 'Matched', fss.map( function (fs) { return fs.name } ), 'silly' )
	return fss
}
...
barrel.appease = function ( comm, err, responseComms ) {
	...
	self.matchingResponse( comm ).forEach( function ( fs ) {
		...
		self.logger.harconlog( null, 'Appeasing', { name: fs.name, comm: comm.shallow() }, 'silly' )
...
barrel.intoxicate = function ( comm ) {
	...
	let callChain = matching.map( function ( fs ) {
		return function ( cb ) {
			if ( !fs.concealed && !self.seals.includes( comm.event ) )
