...
barrel.removeEntity = function ( division, name, callback) {
	if (callback)
		callback()
}
barrel.newEntity = function ( division, name, callback) {
	if (callback)
		callback()
}
...
barrel.castOf = function ( name, callback ) {
	...
	if ( index !== -1) {
		...
		async.series( [
			function (cb) {
				self.removeEntity( fs.division, fs.name, cb )
			},
			function (cb) {
				fs.close( cb )
			}
...
barrel.affiliate = function ( firestarter, callback ) {
	...
	async.series( [
		...
		function (cb) {
			self.newEntity( firestarter.division, firestarter.name, cb )
		}
	], callback )
