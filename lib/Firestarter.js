...
let assigner = new Assigner().excluded(['erupt'])
...
firestarter.burn = function ( comm, callback ) {
	...
	if ( serviceInfo.async ) {
		...
		serviceFn.apply( self.object || { comm: comm, ignite: igniteFn }, params )
			.then( (res) => { callbackFn( null, res ) } )
			.catch( (reason) => { callbackFn( reason ) } )
	}
	else {
		...
		let res = serviceFn.apply( self.object || { comm: comm, ignite: igniteFn }, params )
		if ( res && res.catch ) res.catch( (reason) => { self.logger.harconlog( reason ) } )
	}
}
...
firestarter.burst = function ( comm, params ) {
	...
	let callback = hasCallback ? params[ params.length - 1 ] : () => {}
...
firestarter.innerIgnite = function ( ) {
	...
	let callback = hasCallback ? arguments[ arguments.length - 1 ] : () => {}
