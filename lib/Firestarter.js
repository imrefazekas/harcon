...
firestarter.burn = function( comm, callback ){
	...
	var params = [].concat( comm.params ).concat( serviceFn.length > comm.params.length+2 ? comm.externals : [] ).concat( serviceFn.length > comm.params.length+1 ? igniteFn : [] ).concat( callbackFn );
...
firestarter.simpleIgnite = function( ){
	...
	if( externalId && self.externals[externalId] ){
		comm.externals = self.externals[externalId];
		delete self.externals[externalId];
	}
