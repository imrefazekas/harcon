...
firestarter.appease = function ( error, comm, responseComms ) {
	...
	return new Promise( async (resolve, reject) => {
		...
		if ( bcomm ) {
			let response
			try {
				response = self.conformResponse( error, responseComms )
				if (response.err) reject( response.err )
				else resolve( (self.barrel.unfoldAnswer && response.res.length === 1) ? response.res[0] : response.res )
...
*/
firestarter.burn = function ( comm ) {
	let self = this
	return new Promise( async (resolve, reject) => {
		...
		let callbackFn = function (err, res) {
			...
			if (err) reject(err)
			else resolve( comm.twist(self.name, err, res) )
		}

		let paramList = serviceInfo.params
		if ( paramList.length !== comm.params.length )
			return callbackFn( new Error('Mind the parameter list! You have sent ' + comm.params.length + ' instead of ' + paramList.length + ' for ' + self.name + '.' + comm.event ) )
		...
		let params = [].concat( comm.params ).concat( serviceFn.length > comm.params.length + 1 ? terms : [] ).concat( serviceFn.length > comm.params.length ? igniteFn : [] )
		try {
...
firestarter.burst = function ( comm, params ) {
	...
	return new Promise( async function (resolve, reject) {
		try {
			let newComm = comm.burst( externalID, flowID, self.division, self.name, division, event, callParams )
...
firestarter.innerIgnite = function ( ) {
	...
	return new Promise( async function (resolve, reject) {
		try {
			let comm = Communication.newCommunication( null, flowId, externalId, self.division, self.name, division, event, params )
